...
//        Record selectedRecord = recordMapper.selectOne(recordWrapper);
    ...
    public List<Map<String, String>> showEvaluation(int hotelId) {
        ...
        for (Evaluation e : evaluations) {
            ...
            stringStringMap.put("startTime",(record.getBookStartTime().getYear()+1900)+"-"+record.getBookStartTime().getMonth()+"-"+record.getBookStartTime().getDay());
            stringStringMap.put("endTime",(record.getBookEndTime().getYear()+1900)+"-"+record.getBookEndTime().getMonth()+"-"+record.getBookEndTime().getDay());
            Room room = roomMapper.selectOne(new LambdaQueryWrapper<Room>().eq(Room::getId, record.getRoomId()));
